...
pacman::p_load(tidyverse, ggpubr, patchwork, cowplot, GGally)
xTextSize<-14

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

# read in cleaned data
HostRange24.all<-read.csv("GlobiHostRangeData2024.csv", header=TRUE)
#view(HostRange24.all)

# heatmap EOPs
HostRange24.EOPhosts<-HostRange24.all |>
  dplyr::filter(StrainID=="B2880" | StrainID=="B24025" | StrainID=="B24478" | StrainID=="B24479") |>
  mutate(logEOP=log10(EOP)) 

# reorder for plotting
HostRange24.EOPhosts$StrainID <-factor(HostRange24.EOPhosts$StrainID, 
                                       levels=c('B2880', 'B24025', 'B24478', 'B24479'))
  
# Plot heatmap of log10(EOP). NA (no plaque formation recorded) is in grey
pEOPbyPhage.all.hm<-HostRange24.EOPhosts |>
  ggplot(aes(y=Name, x=StrainID, fill=logEOP)) + 
  geom_tile() +
...
pEOPbyPhage.all<-HostRange24.EOP.all %>%
  ...
  geom_jitter(aes(pch=StrainID, color=LFW), width=0.1, size=2)+
  theme_bw()+
...
pEOPbyHost.all<-HostRange24.EOP.all %>%
  #dplyr::filter(Set == 1) %>%
  ggplot(aes(y=logEOP, x=StrainID))+
  geom_boxplot()+
  #geom_violin()+
  geom_jitter(aes(pch=StrainID, color=LFW), width=0.05, size=2)+
...
HostRange24.EOP.globi<-  HostRange24.EOP.all %>%
  dplyr::filter(LFW!="Y") %>%
  dplyr::select(Set, PhageID, Name, StrainID, logEOP) %>%
  dplyr::filter(StrainID=="B24025" | StrainID=="B2880" | StrainID=="B24478") %>%
  pivot_wider(names_from=StrainID, values_from=logEOP)


### stat tests

# Wilcox
wilcox.test(HostRange24.EOP.globi$B2880, HostRange24.EOP.globi$B24478) #p=0.1127
wilcox.test(HostRange24.EOP.globi$B2880, HostRange24.EOP.globi$B24025) #p=0.005128 
wilcox.test(HostRange24.EOP.globi$B24478, HostRange24.EOP.globi$B24025) #p=0.00658

# Linear regressions
lm.2880.24478<-lm(B2880~B24478, data=HostRange24.EOP.globi)
summary(lm.2880.24478)
# n.s.

lm.2880.24025<-lm(B2880~B24025, data=HostRange24.EOP.globi)
summary(lm.2880.24025)
#m = 0.8145, b = -1.6479, p=0.058 on F test
#R2adj = 0.2069

lm.24478.24025<-lm(B24478~B24025, data=HostRange24.EOP.globi)
...
# medians?
median(HostRange24.EOP.globi$B2880[HostRange24.EOP.globi$B2880 > -5]) #-1.255
median(HostRange24.EOP.globi$B24025[HostRange24.EOP.globi$B24025>-5]) #-0.533
...
HostRange24.phagesummary.all<-HostRange24.all %>%
  ...
  group_by(Host, HostSpecies, StrainID) %>%
  summarize(nPhage=n(),
...
pAnyPlaques_bar_all<-HostRange24.phagesummary.all %>%
  dplyr::filter(StrainID != "B2884" & StrainID != "B1814" & StrainID != "B4425") %>%
  ggplot(aes(x=StrainID, y=fAny, fill=factor(StrainID)))+
  geom_col()+
  ylim(0,1)+
  theme_bw()+
...
pIndivPlaques_bar_all<-HostRange24.phagesummary.all %>%
  dplyr::filter(StrainID != "B24479" & StrainID != "B2884" & StrainID != "B1814" & StrainID != "B4425") %>%
  ggplot(aes(x=StrainID, y=fPlaque, fill=factor(StrainID)))+
  geom_col()+
  theme_bw()+
  ylim(0,1)+
  scale_fill_viridis_d(option="magma", end=0.85)+
...
(pAnyPlaques_bar_all | pEOPbyHost.all) / (pEOPbyPhage.all.hm | pHostRange24.EOP.multipanel)  + 
  plot_annotation(tag_levels = 'A') #+
#  plot_layout(guides = 'collect')
ggsave("GlobiPhageHostRangeSummaryPlotTall.png", width=12, height=14, units="in", dpi=500)
